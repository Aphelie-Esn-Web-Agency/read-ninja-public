...
(function () {
	var s = readninjaAdminSettings.current;

	// ---- Build preview container -------------------------------------------

	var container = document.createElement('div');
	container.id = 'readninja-preview';
	...
	bar.id = 'readninja-preview-bar';

	// Free indicator badge (inside bar)
	var badge = document.createElement('span');
	badge.id = 'readninja-preview-badge';
	...
	function apply() {
		...
		// -- Free settings --
		var color    = fieldVal('readninja_settings[color]',    s.color    || '#1D9E75');
		var height   = Math.max(2, parseFloat(fieldVal('readninja_settings[height]',  s.height  || 4)));
		var position = fieldVal('readninja_settings[position]', s.position || 'top');
		var opacity  = parseFloat(fieldVal('readninja_settings[opacity]', s.opacity  || 100)) / 100;
		// 'custom' treated as top for preview purposes
		var isTop    = position !== 'bottom';

		// -- Gradient (Pro) --
		var grad    = s.gradient || {};
		var gradOn  = fieldChecked('readninja_pro_gradient[enabled]', grad.enabled);
		var background;

		if (gradOn) {
			var c1  = fieldVal('readninja_pro_gradient[color_start]', grad.color_start || '#3b82f6');
			var c2  = fieldVal('readninja_pro_gradient[color_end]',   grad.color_end   || '#8b5cf6');
			var dir = fieldVal('readninja_pro_gradient[direction]',   grad.direction   || 'ltr');
			var ang = dir === 'ltr' ? 90
			        : dir === 'rtl' ? 270
			        : (parseInt(fieldVal('readninja_pro_gradient[angle]', grad.angle || 90), 10) || 90);
		...
		var indType   = fieldRadio('readninja_settings[indicator_type]', s.indicator_type || 'none');
		var indActive = indType !== 'none';

		var sizeMode = fieldRadio('readninja_indicator_size_mode',
			(s.indicator_size && s.indicator_size !== 'auto') ? 'custom' : 'auto');
		var sizeInputEl = document.getElementById('readninja_indicator_size_input');
		var indFixedSize = 0;
		if (sizeMode === 'custom') {
		...
		if (indActive) {
			...
			var indColorVal = fieldVal('readninja_settings[indicator_color]', s.indicator_color || 'auto');
			var badgeColor;
			if (indColorVal !== 'auto') {
				badgeColor = indColorVal;
			} else if (gradOn) {
				var c1l = previewLuminance(fieldVal('readninja_pro_gradient[color_start]', '#3b82f6'));
				var c2l = previewLuminance(fieldVal('readninja_pro_gradient[color_end]', '#8b5cf6'));
			...
			var samplePct = '60%';
			var indPrefix = (fieldVal('readninja_settings[time_prefix]', s.time_prefix || '') || '').trim();
			var indSuffix = (fieldVal('readninja_settings[time_suffix]', s.time_suffix || 'min restantes') || '').trim();
			var indFormat = fieldRadio('readninja_settings[time_format]', s.time_format || 'minutes');
			// Simulated remaining: 400 words left at user WPM
			var indWpm    = Math.max(100, parseInt(fieldVal('readninja_settings[wpm]', s.wpm || 200), 10) || 200);
			...
			var indPos = fieldVal('readninja_settings[indicator_position]', s.indicator_position || 'inside');
			if (indPos === 'inside') {
	...
	var form = document.querySelector('.readninja-settings-form') || document.querySelector('form[action*="options.php"]');
	if (form) {
	...
	var indPicker = document.getElementById('readninja_indicator_color_picker');
	if (indPicker) {
		indPicker.addEventListener('input', apply);
	}
	var indSizeInput = document.getElementById('readninja_indicator_size_input');
	if (indSizeInput) {
